...
export default function Home() {
  ...
  const filteredPlaces = useMemo(() => {
    ...
    const matches = places.filter((place) => {
      const matchesCategory =
        activeCategory === 'Tudo' || place.tags.includes(activeCategory);
      const searchableText =
        `${place.name} ${place.type} ${place.detail} ${place.price}`.toLocaleLowerCase(
          'pt-BR',
        );
      return (
        matchesCategory &&
    ...
    return [...matches].sort((a, b) => {
      const parseDistance = (value: string) =>
        Number(
          value.replace(' km', '').replace(',', '.').replace('850 m', '0.85'),
        );
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="relative z-20 border-b border-[#3b1c12]/10 bg-[#fffaf1]/90 backdrop-blur-xl">
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-5 sm:px-8">
          ...
          >
            ...
            <span className="font-heading text-xl font-black tracking-[-0.04em]">
              OHFOME
            </span>
          </a>

          <nav
            className="hidden items-center gap-8 text-sm font-semibold text-[#5b3a2e] md:flex"
          ...
          >
            ...
            <a className="transition-colors hover:text-primary" href="#negocio">
              Para negócios
            </a>
          </nav>

          <Link
            className="inline-flex h-10 items-center gap-2 rounded-full border border-[#32170f]/15 bg-[#fffaf1] px-4 text-sm font-bold transition-colors hover:bg-[#f3e7d4]"
            href="/app"
          >
      ...
      <section
        id="inicio"
        className="relative isolate border-b border-[#3b1c12]/10 bg-[#fffaf1]"
      >
        ...
        <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-14 sm:px-8 sm:py-20 lg:grid-cols-[1.08fr_.92fr] lg:py-24">
          <div className="max-w-2xl">
            <Badge
              className="mb-6 h-7 rounded-full border-[#d4562e]/20 bg-[#f7d778] px-3 text-[#4d281c]"
              variant="outline"
            >
              <Sparkles className="size-3.5" /> Feito para sentir o sabor de
              Teresina
            </Badge>
            ...
            <p className="mt-7 max-w-xl text-lg leading-8 text-[#6b493c] sm:text-xl">
              Descubra pequenos restaurantes, barracas e sabores que contam a
              história de Teresina — perto de você e dentro do seu orçamento.
            </p>
            ...
            >
              ...
              <Button
                className="h-12 rounded-2xl px-6 text-base font-bold shadow-none"
                type="submit"
              >
                Encontrar sabores
              </Button>
          ...
          <div className="relative mx-auto w-full max-w-[570px] lg:mx-0">
            ...
            <div className="relative aspect-[4/5] rotate-1 overflow-hidden rounded-[2.3rem] border-[10px] border-white bg-[#e5bf7f] shadow-[0_35px_90px_rgba(70,28,13,.25)]">
              <Image
              ...
              <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#1d0b06]/90 via-[#1d0b06]/45 to-transparent p-7 pt-24 text-white">
                <p className="text-sm font-semibold text-[#ffd878]">
                  Descoberta do dia
                </p>
                <p className="mt-1 font-heading text-3xl font-black tracking-tight">
                  Sabores do nosso Piauí
                </p>
                <p className="mt-2 flex items-center gap-2 text-sm text-white/80">
                  <MapPin className="size-4" /> Teresina, PI
                </p>
              </div>
            </div>
            ...
            <div className="absolute -bottom-6 -left-3 flex items-center gap-3 rounded-2xl border border-[#32170f]/10 bg-white p-3.5 pr-5 shadow-[0_16px_45px_rgba(70,28,13,.2)] sm:-left-9">
              <span className="grid size-11 place-items-center rounded-xl bg-[#dff0d8] text-[#28533c]">
                <Compass className="size-5" />
              </span>
              <div>
                <p className="text-xs font-semibold text-[#826257]">
                  Perto de você
                </p>
                <p className="font-bold text-[#32170f]">
                  23 lugares para explorar
                </p>
              </div>
            </div>
          </div>
      ...
      <section id="descobertas" className="bg-[#f3e7d4] px-5 py-20 sm:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
            <div>
              <p className="text-sm font-black uppercase tracking-[.16em] text-primary">
                Descobertas agora
              </p>
              <h2 className="mt-2 font-heading text-4xl font-black tracking-[-.045em] text-[#32170f] sm:text-5xl">
                Lugar bom se compartilha.
              </h2>
            </div>
            <button
            ...
            >
              ...
              <ChevronDown
                className={`size-4 transition-transform ${sortClosest ? 'rotate-180' : ''}`}
              />
            </button>
          </div>
          ...
          {filteredPlaces.length > 0 ? (
            <div className="mt-9 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {filteredPlaces.map((place) => (
                <Card
                  className="gap-0 rounded-[1.6rem] border-0 bg-[#fffaf1] py-0 shadow-[0_14px_40px_rgba(72,35,20,.08)] ring-1 ring-[#32170f]/8"
                  key={place.name}
                >
                  <div className="relative aspect-[16/10] overflow-hidden">
                    <Image
                      alt="Ambiente e sabores de um pequeno negócio local"
                    ...
                    />
                    <Badge className="absolute left-4 top-4 h-7 bg-[#fffaf1] text-[#32170f] shadow-sm">
                      {place.type}
                    </Badge>
                    <button
                  ...
                  <CardHeader className="gap-2 px-5 pb-3 pt-5">
                    <div className="flex items-center justify-between gap-3">
                      <CardTitle className="font-heading text-xl font-black tracking-tight text-[#32170f]">
                        {place.name}
                      </CardTitle>
                      <span className="flex items-center gap-1 text-sm font-bold text-[#32170f]">
                        <Star className="size-4 fill-[#f2bd35] text-[#f2bd35]" />{' '}
                        {place.rating}
                      </span>
                    </div>
                    <p className="text-sm leading-6 text-[#79594e]">
                      {place.detail}
                    </p>
                  </CardHeader>
                  <CardContent className="flex items-center gap-3 px-5 pb-5 text-sm font-semibold text-[#4c3027]">
                    <span>{place.price}</span>
                    <span className="size-1 rounded-full bg-[#b89a8d]" />
                    <span>{place.distance}</span>
                  </CardContent>
                  <CardFooter className="justify-between border-[#32170f]/8 bg-[#f8eddc] px-5 py-3.5 text-sm text-[#5d4035]">
                    <span className="flex items-center gap-2">
                      <Clock3 className="size-4 text-[#2f6749]" />{' '}
                      {place.status}
                    </span>
            ...
            <div className="mt-9 rounded-[1.6rem] border border-dashed border-[#32170f]/20 bg-[#fffaf1]/60 px-6 py-16 text-center">
              <p className="font-heading text-2xl font-black text-[#32170f]">
                Ainda não encontramos esse sabor.
              </p>
              <p className="mt-2 text-[#79594e]">
                Tente “comida típica”, “café” ou limpe os filtros.
              </p>
            </div>
          )}

          <p className="mt-5 text-xs text-[#806357]">
            Nomes, avaliações, distâncias e preços são dados demonstrativos para
            este protótipo.
          </p>
        </div>
      </section>
      ...
      <section
        id="como-funciona"
        className="bg-[#fffaf1] px-5 py-20 sm:px-8 sm:py-28"
      >
        <div className="mx-auto max-w-7xl">
          <div className="grid gap-12 lg:grid-cols-[.82fr_1.18fr] lg:items-start">
            <div className="lg:sticky lg:top-10">
              <Badge
                className="h-7 border-[#d4562e]/20 bg-[#f7d778] text-[#4d281c]"
                variant="outline"
              >
                Da vontade ao caminho
              </Badge>
              ...
              <p className="mt-6 max-w-lg text-lg leading-8 text-[#76564a]">
                A demonstração traduz a proposta do projeto em uma jornada
                curta, clara e possível de usar pelo celular.
              </p>
            </div>
              ...
              {[
                {
                  icon: Search,
                  step: '1',
                  title: 'Conte sua vontade',
                  text: 'Digite o prato, o orçamento ou a experiência que procura.',
                },
                {
                  icon: MapPin,
                  step: '2',
                  title: 'Use os filtros',
                  text: 'Aproxime os resultados por preço, distância e preferência local.',
                },
                {
                  icon: Sparkles,
                  step: '3',
                  title: 'Receba sugestões',
                  text: 'Veja lugares compatíveis e recomendações que ficam melhores com o uso.',
                },
                {
                  icon: Route,
                  step: '4',
                  title: 'Escolha e vá',
                  text: 'Confira horário, rota e forma de contato antes de sair.',
                },
              ].map(({ icon: Icon, step, title, text }) => (
                <article
                  className="relative min-h-60 rounded-[1.65rem] border border-[#32170f]/10 bg-[#f7ecda] p-6"
                  key={step}
                >
                  <span className="absolute right-5 top-3 font-heading text-7xl font-black tracking-tighter text-[#d4562e]/10">
                    {step}
                  </span>
                  <span className="grid size-12 place-items-center rounded-2xl bg-[#2e6648] text-[#fffaf1]">
                    <Icon className="size-5" />
                  </span>
                  <h3 className="mt-8 font-heading text-2xl font-black tracking-tight text-[#32170f]">
                    {title}
                  </h3>
      ...
      <section className="border-y border-[#fffaf1]/10 bg-[#2b1510] px-5 py-20 text-[#fffaf1] sm:px-8 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-3xl">
            <p className="text-sm font-black uppercase tracking-[.16em] text-[#f4c84e]">
              Fluxo de desenvolvimento
            </p>
            <h2 className="mt-3 font-heading text-5xl font-black leading-[.96] tracking-[-.055em] sm:text-6xl">
              Da ideia ao piloto na rua.
            </h2>
            <p className="mt-6 max-w-2xl text-lg leading-8 text-[#d6bfb6]">
              Um caminho enxuto para validar valor primeiro e adicionar
              tecnologia à medida que a comunidade usa a plataforma.
            </p>
          </div>
          ...
          <ol className="mt-12 grid gap-px overflow-hidden rounded-[1.8rem] border border-white/10 bg-white/10 lg:grid-cols-5">
            {developmentSteps.map(
              ({ number, title, text, output, icon: Icon }) => (
                <li
                  className="flex min-h-[330px] flex-col bg-[#321a14] p-6 transition-colors hover:bg-[#3a1e17]"
                  key={number}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-mono text-sm font-bold text-[#f4c84e]">
                      {number}
                    </span>
                    <Icon className="size-5 text-[#e06a43]" />
                  </div>
                  <h3 className="mt-12 font-heading text-2xl font-black leading-tight tracking-tight">
                    {title}
                  </h3>
                  <p className="mt-4 text-sm leading-6 text-[#cdb5ac]">
                    {text}
                  </p>
                  <p className="mt-auto flex items-start gap-2 border-t border-white/10 pt-5 text-sm font-semibold text-[#f8e8d4]">
                    <Check className="mt-0.5 size-4 shrink-0 text-[#7eb995]" />{' '}
                    {output}
                  </p>
                </li>
              ),
          ...
          <div className="mt-8 grid gap-4 md:grid-cols-3">
            {[
              ...
              [
                'Métrica principal',
                'Pessoas que descobrem e contatam um local',
              ],
            ].map(([label, value]) => (
              <div
                ...
                key={label}
              >
                <p className="text-xs font-black uppercase tracking-[.14em] text-[#d8896e]">
                  {label}
                </p>
      ...
      <section
        id="negocio"
        className="relative overflow-hidden bg-[#e45734] px-5 py-20 sm:px-8 sm:py-24"
      >
        ...
        <div className="relative mx-auto grid max-w-7xl gap-10 lg:grid-cols-[1.1fr_.9fr] lg:items-center">
          <div>
            <Badge
              className="h-7 border-white/30 bg-white/15 text-white"
              variant="outline"
            >
              <Store className="size-3.5" /> Para pequenos negócios
            </Badge>
            <h2 className="mt-5 max-w-3xl font-heading text-5xl font-black leading-[.92] tracking-[-.06em] text-white sm:text-7xl">
              Seu tempero merece ser encontrado.
            </h2>
          </div>
          <div className="rounded-[1.6rem] bg-[#fffaf1] p-7 text-[#32170f] shadow-[0_24px_70px_rgba(73,19,8,.24)] sm:p-9">
            <p className="text-lg leading-8 text-[#6b493c]">
              O cadastro foi pensado para quem não tem site ou equipe de
              marketing: informações simples, presença digital e contato direto
              com novos clientes.
            </p>
            <ul className="mt-6 space-y-3 text-sm font-semibold">
              {[
                'Perfil gratuito e fácil de atualizar',
                'Horários, preços, fotos e localização',
                'Mais visibilidade para turistas e moradores',
              ].map((benefit) => (
                <li className="flex items-center gap-3" key={benefit}>
                  <span className="grid size-6 place-items-center rounded-full bg-[#dcead8] text-[#2e6648]">
      ...
      <footer className="bg-[#fffaf1] px-5 py-10 sm:px-8">
        <div className="mx-auto flex max-w-7xl flex-col gap-6 border-t border-[#32170f]/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-2.5">
            <span className="grid size-9 place-items-center rounded-xl bg-[#32170f] text-[#fffaf1]">
          ...
          <p className="max-w-xl text-sm leading-6 text-[#79594e]">
            Demonstração acadêmica de uma plataforma que valoriza a culinária, a
            cultura e os pequenos negócios de Teresina.
          </p>
          <span className="flex items-center gap-2 text-sm font-bold text-[#2e6648]">
            <WandSparkles className="size-4" /> Protótipo 2026
          </span>
        </div>
      </footer>
    </main>
